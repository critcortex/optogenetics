...
expbase = '140724_cell_abstract_vitro'
...
Abstract_areas = ['soma','dend0','dend1','dend2','dend3']
areas = {'L5PC': L5PC_areas,
         'FractalNeuron': Abstract_areas,
         'SHStellate':SHStellate_areas}

celltype = 'L5PC'
celltype = 'SHStellate'
celltype = 'FractalNeuron'
#(4,5,3)
nb = 4
nc = 5
nl = 3
# Taken directly from 140305_compare_equal.py
cellparams = {'num_base': nb,
                      'num_split': nc,
                      'num_levels':nl,
...
def get_marked_locs(labels,nsites):
    markedloc = {}
    if celltype == 'L5PC':
        markedloc['names'] = ['mysoma','distal']+labels
        markedloc['sections'] = ['soma','apic']+['apic']*nsites
        markedloc['ids'] = [(0,0.5),(0,0.972326)] + [('select_section_posn_bydistance',{'sectionarea':'apic','mindist':dist[0],'maxdist':dist[1]}) for i in range(nsites)] 
    elif celltype == 'SHStellate':
        markedloc['names'] = ['mysoma','distal']+labels[:-1]
        markedloc['sections'] = ['soma']+['dendrite']*nsites
        markedloc['ids'] = [(0,0.5)] + [('select_section_posn_bydistance',{'sectionarea':'dendrite','mindist':dist[0],'maxdist':dist[1]}) for i in range(nsites)] 
    elif celltype == 'FractalNeuron':
        sec_length = 50
        dist = [sec_length*(nl+0.5),sec_length*(nl+0.55)]
        markedloc['names'] = ['mysoma','distal']
        markedloc['sections'] = ['soma']+['dend0']
        markedloc['ids'] = [(0,0.5)] + [('dend0_0_0',0.5)] 
            ...
            for irr in irrs: 
                for (i,J) in enumerate(Js):
                    for nsites in nsite_range:
                        ...
                        print labels,locs
                        pp['mark_loc'] = {}
                        pp['mark_loc']['names'] = ['mysoma','distal']
                        pp['mark_loc']['sections'] = ['soma']+['dend0']
                        pp['mark_loc']['ids'] = [(0,0.5)] + [('get_section_byname',{'sectioname':'dend0_0_0','subdomain':'dend0'})]
                        ...
                        pp.update({'expname':expbase,
                                   'description':'irr%.3f_factor%.2f_freq%g_J%g_nsites%g_celltype%s'%(irr,factor,freq,J,nsites,celltype)})
    ...
    for factor in factors:
        for irr in irrs: 
            for Ia in iclamp_amps:
                ...
                pp['mark_loc']['names'] = ['mysoma','distal']
                pp['mark_loc']['sections'] = ['soma']+['dend0']
                pp['mark_loc']['ids'] = [(0,0.5)] + [(2,0.5)] # [('get_section_byname',{'sectioname':'dend0_0_0','subdomain':'dend0'})]
                ...
                pp.update({'expname':expbase,
                           'description':'irr%.3f_factor%.2f_I%.2f_stimloc_%s_celltype%s'%(irr,factor,Ia,stimloc,celltype)})
...
def analyse_locations_optogen_invivo():
    ...
    for f in factors:
        ...
        af.submenu_plot(5, expbase+'FI_gain_varyIrr_invivo')
        af.submenu_plot(0, expbase+'FI_gain_varyIrr_invivo')
        af.submenu_plot(10, expbase+'FI_gain_varyIrr_invivo')
...
def analyse_locations_optogen_invitro():
    ...
    for f in factors:
        ...
        exp_comp_list = [['irr%.3f_'%irr+'factor%.2f_'%f+'I%.2f'+'_stimloc_distal_celltype%s'%celltype+'_NpHR_%s_ChR_%s'%(get_optdescript(irr,f)[1],get_optdescript(irr,f)[0]),'%.3f'%irr] for irr in irrs]
...
if __name__ == '__main__':
    if len(sys.argv) <= 1:
        ...
        scan_locations_optogen_invitro('distal')        
