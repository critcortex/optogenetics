"""
Based on 140723 but with inhibitory as well as excitatory synapses
...
light_on = 1200
light_dur = 1000
tstop = light_on + light_dur + 300 # as buffer
...
Js = [2.]
nsite_range = [80]
ei_ratio_range = [0.05,0.1,0.15,0.2]
inh_correction = []
dist = [100,-1]
...
# debug params
irrs = [0.005] #[0.001,0.002]
#freqs = [50]
#factors = [0.5]

ei_ratio_range = [0.05,0.1,0.15,0.2]
inh_corrections = [0.5,0.8,1.,1.2]
            ...
            for irr in irrs: 
                for (i,J) in enumerate(Js):
                    for nsites in nsite_range:
                        for ei_ratio in ei_ratio_range: 
                            for inh_correction in inh_corrections:
                    
                                if freq == 0 and i > 0:
                                    continue
                                
                                nInh = int(math.ceil(ei_ratio*nsites))
                                g = nsites/nInh
                                Ji = -1*J*inh_correction*g
                                ...
                                labelsInh = ['stim_i_%g'%i for i in range(nInh*2)]
                                
                                pp['mark_loc'] = {}
                                pp['mark_loc']['names'] = ['mysoma']+labels+labelsInh
                                pp['mark_loc']['sections'] = ['soma']+['apic']*nsites+['dend']*nsites+['apic']*nInh+['dend']*nInh
                                pp['mark_loc']['ids'] = [(0,0.5)] + [('select_section_posn_bydistance',{'sectionarea':'apic','mindist':dist[0],'maxdist':dist[1]}) for i in range(nsites)] + [('select_section_posn_bydistance',{'sectionarea':'dend','mindist':dist[0],'maxdist':dist[1]}) for i in range(nsites)] + [('select_section_posn_bydistance',{'sectionarea':'apic','mindist':dist[0],'maxdist':dist[1]}) for i in range(nInh)] + [('select_section_posn_bydistance',{'sectionarea':'dend','mindist':dist[0],'maxdist':dist[1]}) for i in range(nInh)] 
                                ...
                                pp['spiketrains'] = [{'tstims': get_spiketimes(freq,nsites*2), 'locations': labels, 'weights':np.ones(2*nsites)*J,  'el': 0.1}] + [{'tstims': get_spiketimes(freq,2*nInh), 'locations': labelsInh, 'weights':np.ones(2*nInh)*Ji,  'el': 0.1}]
                                ...
                                pp.update({'expname':expbase,
                                           'description':'irr%.3f_factor%.2f_freq%g_J%g_nsites%g_nI%g_Ji%.2f'%(irr,factor,freq,J,2*nsites,2*nInh,Ji)})
...
def analyse_locations_optogen_invivo():
    ...
    for f in factors:
        ...
        af.submenu_plot(0, expbase+'FI_gain_irr%.2f_tree%s_varyFactor_exp%s%s_'%(0.05,tree,exp[0],exp[1]))
...
def analyse_factors_invivo():
    ...
    for irr in [0.005]: #irrs:
        ...
        af.update_params({'tstart':light_on,'tstop':light_on+light_dur,
                              ...
                              'tstart_post':light_on+light_dur,'tstop_post':tstop})
        #optlog = get_optdescript(irr,1)
        exp_comp_list = [['irr%.3f_'%irr+'factor%.2f_'%f+'freq%g'+'_J%g_nsites%g_nI%g_Ji%.2f'%(Js[0],nsite_range[0]*2,32,10)+'_NpHR_%s_ChR_%s'%(get_optdescript(irr,f)[1],get_optdescript(irr,f)[0]),'%.2f'%f] for f in factors]
        ...
        for exp in af.experimentset:
            ...
            exp.calculate_responses('FI_bg')
            exp.calculate_responses('FI_post')
            exp.calculate_responses('cv_isi')
            exp.calculate_responses('fano_factor_isi')
