...
class Experiment_151103:
    ...
    def __init__(self,whole,stimulus):
        ...
        self.expbase = '151223_plotIntrinsicAbstract_pretty'
    ...
    def setIntrinsic(self):
        self.collection_same_total = self.collection_tree #collection_most
    ...
    def setGainCheck(self):
        self.collection_same_total = self.collection_tree
        self.factors = [0.25,0.5,0.75,1.]#,1.25,1.5,2.,4.]
        self.irrs = [ a*b for a in [2.0] for b in [0.01,0.1]]
    ...
    def runGainCheckPartial(self):
        # as for setIntrinsic()
        self.collection_same_total = [(4,5,3),(2,2,6),(4,1,31)]
        self.factors = [0.25,0.5,0.75,1.,1.25,1.5,2.,4.]
        self.irrs = [ a*b for a in [2.0] for b in [0.01]]
        self.freqs = range(10,101,10)
        self.whole = False
            ...
            for factor in self.factors:
                for irr in self.irrs: 
                    ...
                    pp.update({'expname':self.expbase,
                               'description':'irr%.3f_factor%.2f_nb%g_ns%g_nl%g_Intrinsic'%(irr,factor,nb,nc,nl)})
                    ...
                    for freq in self.freqs:
                        ...
                        for clamploc in [nl-1]: # distal location
                            ...
                            if checkPartial:
                                ...
                                for check_branches in range(1,nb):
    ...
    def analyse_gainCheck(self):
        ...
        for tree in self.collection_same_total:
            ...
            for irr in self.irrs:
                ...
                descript = 'distal'
                af.submenu_plot(5, self.expbase+'_irr%.3f_nb%g_ns%g_nl%g_'%(irr,tree[0],tree[1],tree[2])+'_inj%s'%(descript))
                af.submenu_plot(0, self.expbase+'_all_FI_gain_irr%.3f_nb%g_ns%g_nl%g'%(irr,tree[0],tree[1],tree[2]))
                af.submenu_plot(10, self.expbase+'bg_irr%.3f_nb%g_ns%g_nl%g_'%(irr,tree[0],tree[1],tree[2])+'inj%s'%(descript))
