...
expbase = '130710_increasing_uneq_irradiance_factor_somacurr'
        ...
        for irr in irradiances:
            for factor in factors:
                for amp in current_amps:
                    ...
                    params['iclamp_dist_amp']= amp
                    params['iclamp_dist_start']= curr_on
                    params['iclamp_dist_duration'] = curr_dur                    
                    ...
                    params.update({'NpHR_times':opsin_expression_np,'ChR_times':opsin_expression_ch,'description':'_irr%.1f_factor%.2f_Idist%.1f'%(irr,factor,amp)})
...
def analyse():
    ...
    exp_comp_list = [['_irr%.1f_factor%.2f_Idist%.1f_'+'NpHR_%s_ChR_%s'%(exp[1],exp[0]),'ChR %s, NpHR %s'%(exp[0],exp[1])] for exp in explist ]
    print exp_comp_list
...
def plot_gain():
    ...
    for irr in [1]: 
        for exp in explist:
            ...
            exp_comp_list = [['_irr%.1f'%(irr)+'_factor%.2f'%(factor)+'_Idist%.1f_'+'NpHR_%s_ChR_%s'%(exp[1],exp[0]),'%g'%factor] for factor in factors]
            print exp_comp_list
