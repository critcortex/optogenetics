...
#define alphaexp alphaexp_CaT
 double alphaexp = 1;
#define betaexp betaexp_CaT
 double betaexp = 1;
#define eca eca_CaT
 double eca = 135;
#define hon hon_CaT
...
#define usetable usetable_CaT
 ...
 "mon_CaT", &mon_CaT,
 "hon_CaT", &hon_CaT,
 "alphaexp_CaT", &alphaexp_CaT,
 "betaexp_CaT", &betaexp_CaT,
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 CaT /home/sjarvis1/git/optogenetics/models/purkinje_ppr/x86_64/CaT.mod\n");
...
static int _hoc_states() {
 ...
 static double _mfac_rates, _tmin_rates;
  ...
  if (_maktable) { double _x, _dx; _maktable=0;
   _tmin_rates =  - 400.0 ;
   _tmax =  300.0 ;
   _dx = (_tmax - _tmin_rates)/2100.; _mfac_rates = 1./_dx;
   for (_i=0, _x=_tmin_rates; _i < 2101; _x += _dx, _i++) {
...
static int  _f_rates ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
 ...
 _lq10 = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   ...
   _lalpha = 0.0025 / ( 1.0 + ( alphaexp * exp ( ( _lv + 40.0 ) / 8.0 ) ) ) ;
   _lbeta = 0.19 / ( 1.0 + ( betaexp * exp ( ( _lv + 50.0 ) / ( - 10.0 ) ) ) ) ;
   _lsum = _lalpha + _lbeta ;
   hinf = _lalpha / _lsum ;
...
static _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
   _t_minf = makevector(2101*sizeof(double));
   _t_mexp = makevector(2101*sizeof(double));
   _t_hinf = makevector(2101*sizeof(double));
   _t_hexp = makevector(2101*sizeof(double));
_first = 0;
}
