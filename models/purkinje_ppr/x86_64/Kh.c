	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 2);
 	...
 	ivoc_help("help ?1 Kh /home/sjarvis1/git/optogenetics/models/purkinje_ppr/x86_64/Kh.mod\n");
...
static int  states ( _p, _ppvar, _thread, _nt ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; {
   ...
   m = mon * ( 0.8 * ( m + mexp * ( minf - m ) ) ) + hon * ( 0.2 * ( m + nexp * ( minf - m ) ) ) ;
    return 0; }
...
static int _ode_count(_type)int _type; { hoc_execerror("Kh", "cannot be used with CVODE");}
...
static _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
   ...
   _t_mexp = makevector(701*sizeof(double));
