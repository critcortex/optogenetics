...
#define kt _p[3]
#define kd _p[4]
#define icaadjust _p[5]
#define capump _p[6]
#define cabuff _p[7]
#define camolflux _p[8]
#define cai _p[9]
#define Dcai _p[10]
#define ica _p[11]
#define drive_channel _p[12]
#define drive_pump _p[13]
#define v _p[14]
#define _g _p[15]
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 cad /home/sjarvis1/git/optogenetics/models/purkinje_ppr/x86_64/CalciumP.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   drive_channel = - ( 10000.0 ) * ( ica * icaadjust ) / ( 2.0 * FARADAY * depth ) ;
   if ( drive_channel <= 0. ) {
     drive_channel = 0. ;
     }
   drive_pump = - kt * cai / ( cai + kd ) ;
   Dcai = ( camolflux * drive_channel ) + ( capump * drive_pump ) + ( cabuff * ( ( cainf - cai ) / taur ) ) ;
   }
 return _reset;
}
 ...
 drive_pump = - kt * cai / ( cai + kd ) ;
 Dcai = Dcai  / (1. - dt*( ( (cabuff)*(( ( ( ( - 1.0 ) ) ) / taur )) ) )) ;
}
 /*END CVODE*/
