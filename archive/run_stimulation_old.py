...
def set_distal_proximal ():
    h('objref sl')
    print distalpoint, proximalpoint
    ...
    maxdiam = 0
    j= 0
    for i in range(int(axsites)):
        dd1 = h.sl.object(i).x[1]
        print 'poss dist = ',int(h.sl.object(i).x[0])        
    ...
    print 'distal ===',h.sl.object(j).x[0], h.sl.object(j).x[1] 
    #h('double site_dend_distal[2]')
    ...
    for i in range(int(axsites)):
        dd1 = h.sl.object(i).x[1]
        print 'poss prox = ',int(h.sl.object(i).x[0])
        dd = h.L5PC.apic[int(h.sl.object(i).x[0])](dd1).diam
        if (dd > maxdiam):
            j = i
            maxdiam = dd 
    ...
    print 'prox ===',h.sl.object(j).x[0], h.sl.object(j).x[1] 
